...
#define debug_print(x,arg...) printf(x,##arg)
#else 
...
static uint8_t sd_get_cardinfo(SD_CardInfo *cardinfo)
{
	...
	{
		printf("[MaixPy] %s | sd_get_csdregister failed\r\n",__func__);
		return 0xFF;
	}
	if (sd_get_cidregister(&(cardinfo->SD_cid)))
	{
		printf("[MaixPy] %s | sd_get_cidregister failed\r\n",__func__);
		return 0xFF;
	}
	if(2 == sd_version)
	{
...
uint8_t sd_init(void)
{
	...
	cardinfo.active = 0;
	fpioa_set_function(TF_CK_PIN, FUNC_SPI1_SCLK);
    fpioa_set_function(TF_SI_PIN, FUNC_SPI1_D0);
    fpioa_set_function(TF_SO_PIN, FUNC_SPI1_D1);
	fpioa_set_function(TF_CS_PIN, FUNC_GPIOHS0 + SD_CS_PIN);
